...
class Lists extends Base
{
    public function index()
    {
        $firstCatIds = [];
        // 思路 首先需要一级栏目
        $categorys = model("Adminabout")->getNormalCategoryByParentId();
        ...
        if(in_array($id, $firstCatIds)) { // 一级分类
            ...
            // 获取二级分类的数据
            $category = model('Adminabout')->get($id);
        ...
        //获取父类下的所有 子分类
        if($categoryParentId) {
            $sedcategorys = model('Adminabout')->getNormalCategoryByParentId($categoryParentId);
        }
        $orders = [];
        // 排序数据获取的逻辑
